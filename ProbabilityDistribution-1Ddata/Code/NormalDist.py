...
if __name__ == "__main__":
   ...
   # plotData2D(X, 'plotWH.pdf')
    ...
    hstd=np.std(hs)
    l = hs * 0.0
    h = np.linspace(hmean - 5 * hstd, hmean + 5 * hstd, 100) #setting the range
    axs.set_ylim(0.00,0.06)
    plt.scatter(hs, l, color='b', label='data')
    plt.plot(h,1/(hstd *np.sqrt(2*np.pi))*np.exp(-(h-hmean)**2/(2*hstd**2)),color='y',label='normal')
